...
const getImages = (query) => {
  ...
  fetch(
    ...
    .catch((err) => {
      errorMassage.innerHTML = `<h3>Sorry somethings wrong please try again!</h3>`;
      handleSpinner();
    });
};
...
const showImages = (images) => {
  ...
  images.forEach((image) => {
    ...
    div.className = "col-lg-2 col-md-4 col-xs-6 img-item mb-2";
    div.innerHTML = ` <img class="img-fluid img-thumbnail" onclick=selectItem(event,"${image.webformatURL}") src="${image.webformatURL}" alt="${image.tags}">`;
    gallery.appendChild(div);
  });
};
//image count (How many Images are selected)
let slideIndex = 0;
...
const createSlider = () => {
  ...
  `;
    ...
    alert("Please put maximum time: At list  500!");
  }
};
